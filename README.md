...
1. Нажать на меню "Каталог курсов".
    - Ожидаемый результат: Открывается каталог курсов, URL страницы не меняется.
...
3. Вбить запрос "Тестирование" в поисковик курсов и выбрать "Инженер по тестированию" из выпадающих вариантов.
...
1. Выбрать блок "Программирование" из предоставленных карточек каталога курсов.
    - Ожидаемый результат: Пользователь переходит на страницу [https://netology.ru/development](https://netology.ru/development).
2. Вбить запрос "Тестирование" в поисковик курсов и выбрать "Инженер по тестированию" из выпадающих вариантов.
    - Ожидаемый результат: Пользователь переходит на страницу [https://netology.ru/programs/qa-middle](https://netology.ru/programs/qa-middle).
...
2. Ввести валидный номер телефона в формате +7XXXXXXXXXX (например, "+79992223344", сгенерированный через Faker).
...
1. Ввести в поле "Имя" невалидные данные (например, "123", сгенерированные через Faker).
2. Заполнить остальные поля валидными данными.
3. Нажать кнопку "Записаться".
    - Ожидаемый результат: Ошибка "Должно состоять из букв" под полем "Имя".

#### Сценарий 10. Невалидный номер телефона
1. Ввести в поле "Телефон" невалидные данные (например, "00000", сгенерированные через Faker).
2. Заполнить остальные поля валидными данными.
3. Нажать кнопку "Записаться".
    - Ожидаемый результат: Ошибка "Неверный формат" под полем "Телефон".

#### Сценарий 11. Невалидный email
1. Ввести в поле "Email" невалидные данные (например, "aa", сгенерированные через Faker).
2. Заполнить остальные поля валидными данными.
3. Нажать кнопку "Записаться".
